...
if sys.version_info[0] < 3:
    exec("""def _exec_code_object(code_object, globals_dict):
    exec code_object in globals_dict
""")
else:
    def _exec_code_object(code_object, globals_dict):
...
class _OutputCapture(object):
    ...
    def flush(self):
        if self.pending:
            self.output_lines.append(self.pending)
            self.pending = ''
...
def _resolve_mcp_home():
    ...
    try:
        ...
        pass
    ascii_home = r'C:\abaqus-mcp'
    if os.path.exists(os.path.join(ascii_home, 'stop_mcp.py')):
        return ascii_home
...
def execute_script(script_content, script_id):
    ...
    output_lines = []
    output_capture = _OutputCapture(output_lines)
    old_stdout = sys.stdout
    old_stderr = sys.stderr

    try:
        with io.open(script_path, 'r', encoding='utf-8') as f:
            code_object = compile(f.read(), script_path, 'exec')
        sys.stdout = output_capture
        sys.stderr = output_capture
        _exec_code_object(code_object, exec_globals)
